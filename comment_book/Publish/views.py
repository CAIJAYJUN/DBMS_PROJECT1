...
def addPublish(request):
    ...
    if request.method == "POST":
        ...
        insertData = Publisher.objects.create(
            id_num=id_num,
...
def updatePublish(request):
    ...
    if request.method == "POST":
        ...
        if len(request.POST["PasswordUpdate"]) > 0:
            Publisher.objects.filter(name=name).update(
                id_num=email, passWord=make_password(pwd), tel=tel, emailAddr=email
            )
        else:
            Publisher.objects.filter(name=name).update(
                id_num=email, tel=tel, emailAddr=email
            )
...
def book_management(request):
    ...
    return render(
        request, "publisherManagement.html", locals()
    )  # publisherManagement.html 會被塞入多個功能
...
def addAuthor(request):
    ...
    if request.method == "POST":
        ...
        insertData = Author.objects.create(
            id_num=id_num, name=name, sex=sex, authorProfile=desc, birthYear=year
        )
        insertData.save()
