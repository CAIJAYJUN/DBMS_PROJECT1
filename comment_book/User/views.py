...
def register(request):
    ...
    if request.method =='POST':
        ...
        insertData=User.objects.create(id_num=id_num,passWord=make_password(pwd),name=name,location=addr,birthYear=year,indentify=1)
        insertData.save()
        request.session.flush()
        return redirect('/comment_book/register/')
...
def register_admin(request):
    ...
    if request.method =='POST':
        ...
        addr=request.POST['address']
        year=request.POST['year']
        insertData=User.objects.create(id_num=id_num,passWord=make_password(pwd),name=name,location=addr,birthYear=year,indentify=0)
        insertData.save()
        request.session.flush()
        return redirect('/comment_book/registerAdmin/')
